...
class UserPolicySettingsTests(unittest.TestCase):
    def test_safe_is_default_policy(self):
        engine = RiskEngine()

        self.assertEqual(engine.user_policy.mode, "safe")
        self.assertTrue(engine.user_policy.demo_only_default)
        self.assertEqual(engine.user_policy.gemini_role, "confirmation-required")
        self.assertFalse(engine.user_policy.allow_overnight_holding)
        self.assertEqual(engine.settings.policy_mode, "safe")
        self.assertAlmostEqual(engine.settings.max_margin_utilization_pct, 12.0)
